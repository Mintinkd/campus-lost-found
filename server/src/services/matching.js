const { Item, SearchRecord, sequelize } = require('../models');
...
function parseNaturalLanguage(text) {
  ...
  const timePatterns = [
    ...
    { regex: /(\d+)月(\d+)日?/, value: 'specific_date' },
...
function calculateFeatureMatch(searchColor, searchBrand, item) {
  ...
  if (searchColor) {
    factors++;
    if (item.description && item.description.includes(searchColor)) {
      score += 1.0;
    } else {
      score += 0.1;
    }
  ...
  if (searchBrand) {
    factors++;
    if (item.description && item.description.includes(searchBrand)) {
      score += 1.0;
    } else {
      score += 0.1;
    }
...
function calculateMatchScore(dimensions, item) {
  const categoryScore = calculateCategoryMatch(dimensions.category, item.category);
  const locationScore = calculateLocationMatch(dimensions.location, item.location);
  const timeScore = calculateTimeMatch(dimensions.time, item.foundTime);
  const featureScore = calculateFeatureMatch(dimensions.color, dimensions.brand, item);
...
async function semanticSearch(userId, searchText) {
  ...
  if (dimensions.time) {
    const now = new Date();
    switch (dimensions.time.type) {
      case 'today':
        whereClause.foundTime = { [Op.gte]: new Date(now.setHours(0, 0, 0, 0)) };
        break;
      case 'yesterday': {
        const yesterday = new Date(now);
        yesterday.setDate(yesterday.getDate() - 1);
        whereClause.foundTime = {
          [Op.gte]: new Date(yesterday.setHours(0, 0, 0, 0)),
          [Op.lt]: new Date(now.setHours(0, 0, 0, 0))
        };
        break;
      }
      case 'last_week': {
        ...
        whereClause.foundTime = { [Op.gte]: weekAgo, [Op.lt]: weekStart };
        break;
      }
    }
  }
  ...
  const items = await Item.findAll({
    where: whereClause,
    include: [{ model: require('../models').User, as: 'finder', attributes: ['id', 'nickname', 'avatarUrl'] }],
...
async function checkAndNotifyMatch(item) {
  const { SearchRecord, User, Notification } = require('../models');
