...
const { getDecryptedMaskedPhone, decrypt } = require('../utils/crypto');
...
async function createClaim(claimerId, itemId, claimReason) {
  ...
  const todayClaims = await Claim.count({
    where: {
      claimerId,
  ...
  if (todayClaims >= MAX_DAILY_CLAIMS) {
    return { error: '操作频繁，请明日再试', code: 4001 };
  }

  const item = await Item.findByPk(itemId);
  if (!item) {
    return { error: '物品不存在', code: 4002 };
  }

  if (item.status === 'returned' || item.status === 'expired') {
    return { error: '该物品无法认领', code: 4003 };
  }
  ...
  const existingClaim = await Claim.findOne({
    where: {
      itemId,
      claimerId,
      status: { [Op.in]: ['pending', 'confirmed', 'returning'] }
    }
  });

  if (existingClaim) {
    return { error: '您已对该物品发起过认领', code: 4004 };
  }
  ...
  if (activeClaim) {
    return { error: '该物品正在认领中，请稍后重试', code: 4005 };
  }
...
async function rejectClaim(claimId, finderId) {
  ...
  const otherPending = await Claim.count({
    where: { itemId: claim.itemId, status: 'pending' }
  });

  if (otherPending === 0) {
    await Item.update({ status: 'pending' }, { where: { id: claim.itemId } });
  }
...
async function confirmReturn(claimId, finderId) {
  ...
  if (!claim) return { error: '认领记录不存在', code: 4006 };
  if (claim.item.finderId !== finderId) return { error: '无权操作', code: 4007 };
  if (claim.status !== 'confirmed' && claim.status !== 'returning') {
    return { error: '认领状态不正确', code: 4008 };
  }
...
async function getClaimContact(claimId, userId) {
  ...
  if (claim.status !== 'confirmed' && claim.status !== 'returning') {
    return { error: '认领尚未确认', code: 4010 };
  }

  if (isFinder) {
    const claimerPhone = claim.claimer.phoneEncrypted;
    return {
      nickname: claim.claimer.nickname,
      avatarUrl: claim.claimer.avatarUrl,
      phone: claimerPhone ? getDecryptedMaskedPhone(claimerPhone) : null
    };
  } else {
    const finderPhone = claim.item.finder.phoneEncrypted;
    return {
      nickname: claim.item.finder.nickname,
      avatarUrl: claim.item.finder.avatarUrl,
      phone: finderPhone ? getDecryptedMaskedPhone(finderPhone) : null
    };
  }
}
...
async function expireOldClaims() {
  ...
  const expiredClaims = await Claim.findAll({
    where: {
      status: 'pending',
